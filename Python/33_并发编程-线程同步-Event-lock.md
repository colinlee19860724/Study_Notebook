...
　　我们认为结果应该是 0，但是结果可能不如人意，因为进程间共享数据的问题，多个进程同时修改共享数据时，由于 GIL 的存在同一时刻只有 1 个线程在运行。当 n 的值为 1 的时候，10 个子线程很有可能同时判断成功，再要修改的时候被挂起(时间片用完)，等到真正回来修改的时候，n 已经被其他线程改过来！所以如果要保持数据的正确性，那么就需要牺牲性能，即使用锁机制。  
